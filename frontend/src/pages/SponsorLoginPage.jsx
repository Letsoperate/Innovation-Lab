...
const SponsorLoginPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setError("");
    try {
      const res = await api.post("/sponsor/login", { email, password });
      localStorage.setItem("sponsorToken", res.data.token);
      localStorage.setItem("sponsorName", res.data.name);
      localStorage.setItem("sponsorId", res.data.sponsor_id);
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8">
          <div className="text-center mb-6">
            ...
            <p className="text-sm text-gray-500 mt-1">Please enter your sponsor details to login</p>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="sponsor@company.co.za"
        ...
        <p className="text-center text-[11px] text-gray-400 mt-6">
          Sponsor access is managed by Innovation Lab administrators.
        </p>
      </div>
    </div>
  );
